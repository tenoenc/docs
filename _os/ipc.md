...
-   **핵심:** `mmap()` 호출이 성공한 뒤에는 커널을 거치지 않고(Zero System Call) 데이터를 쓴다.
-   **의의:** 마치 내 프로그램의 변수에 값을 대입하는 것과 똑같은 속도로 IPC 통신이 가능함을 의미한다. 대용량 데이터 처리에 압도적으로 유리한다.
...
>
> 이 로컬 IPC 개념은 현대적인 **MSA(Microservices Architecture)** 환경의 통신 방식과 정확히 매핑된다.
>
> 1. **Shared Memory 모델 → Redis 캐시 공유 / Shared DB**
>     - 여러 마이크로서비스(Process)가 **Redis**나 **DB**라는 외부 저장소(Shared Memory)를 통해 상태를 공유한다.
>     - **장점:** 서비스 A가 데이터를 넣으면 서비스 B가 즉시 볼 수 있어 속도가 빠르다.
>     - **Pain Point:** 동시성 문제가 그대로 발생한다. 두 서비스가 동시에 재고를 차감하려 할 때 **분산 락(Distributed Lock)**이 필수적인 이유는 OS의 공유 메모리 동기화 문제와 같다.
>
> 2. **Message Passing 모델 → REST API, gRPC, Message Queue**
>     - **REST/gRPC:** 소켓(Socket) 통신을 기반으로 하며, 명시적으로 요청과 응답을 주고받는다.
>     - **Message Queue (RabbitMQ, Kafka):** OS의 메일박스(Mailbox) 역할을 하는 큐를 통해 비동기로 메시지를 주고받는다.
>     - **특징:** 메모리를 공유하지 않으므로 서비스 간 **결합도(Coupling)**가 낮아지고, 서로의 내부 상태를 모르기 때문에 더 안전한 아키텍처가 된다.
>
> **결론:** "MSA에서 왜 DB를 공유하지 말고 API로 통신하라고 할까?"
> 이는 **"공유 메모리는 빠르지만 위험하고(결합도 높음), 메시지 패싱은 느리지만 안전하다(결합도 낮음)"**는 OS의 오랜 교훈을 분산 시스템에 적용한 것이다. (Database per Service 패턴)
{: .block-tip }