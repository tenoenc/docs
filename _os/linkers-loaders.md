...
>
> 일반적인 OS의 링킹/로딩 과정과 Java의 실행 방식은 결정적인 차이가 있다. 이를 비교하여 정리하는 것이 중요한다.
>
> 1. **컴파일 결과물:** C언어는 OS가 바로 실행할 수 있는 `Native Code`로 컴파일되지만, Java는 `Bytecode(.class)`로 컴파일된다.
> 2. **링킹 시점:** C언어는 주로 실행 전(Build Time)에 링킹이 완료되지만, Java는 **런타임(Runtime)**에 링킹과 로딩이 일어난다.
> 3. **ClassLoader의 마법:**
>     - Java는 `ClassLoader`가 필요한 클래스를 그때그때 동적으로 메모리(Method Area)에 로드한다(**Dynamic Loading**).
>     - **장점:** 이 덕분에 Java 애플리케이션은 멈추지 않고도 새로운 기능을 추가(Plugin 시스템)하거나, 구현체를 갈아끼우는 유연성을 가진다.
>     - **Spring Framework:** 스프링 빈(Bean)이 생성되고 의존성이 주입되는 과정도 결국 이 동적 로딩 메커니즘 위에서 동작하는 것이다.
{: .block-tip }

#### [Practice] Dynamic Loading (Java) vs Static Linking (C)

C언어(Static Linking)는 `printf` 같은 라이브러리 코드를 실행 파일 안에 꽉 채워 넣는다. 반면, Java는 프로그램이 실행되는 도중(Runtime)에 필요한 클래스 파일을 찾아서 메모리에 올리는 **동적 로딩(Dynamic Loading)** 방식을 사용한다.

##### 1. 검증 코드 (Java)

`Class.forName()`을 사용하여, 컴파일 시점에는 존재조차 몰랐던 클래스(문자열로 이름 전달)를 런타임에 로딩하고 인스턴스화하는 과정을 확인한다.
...
-   **지연 로딩 (Lazy Loading):** `Class.forName()`이 호출되기 전까지 `DynamicPlugin` 클래스는 메모리에 존재하지 않는다. 즉, **사용하지 않는 코드는 메모리를 점유하지 않는다**는 OS의 효율적 메모리 관리 원칙을 따른다.
-   **유연성:** 코드를 수정하고 재컴파일하지 않아도, 설정 파일(String)만 바꾸면 실행되는 객체를 교체할 수 있다. (예: JDBC Driver 로딩).

동적 로딩에 대해 보다 더 자세한 예시는 [Runtime Constant Pool Resolution](https://tenoenc.github.io/posts/runtime-data-area/#runtime-constant-pool-resolution)을 참고한다.

> **백엔드 관점: Spring Bean과 동적 로딩**
>
> 우리가 사용하는 **Spring Framework**의 근간이 바로 이 기술이다. `@Component`가 붙은 클래스들을 스캔해서 동적으로 메모리에 올리고, 의존성 주입(DI)을 해준다. 만약 Java가 C언어처럼 정적 링킹만 지원했다면, 빈 하나를 추가할 때마다 서버 전체를 다시 빌드하고 링킹하는 고통을 겪었을 것이다.
{: .block-tip }